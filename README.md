...
<!-- repository-facts: repo=0.8.0; engineering=5.13.0; plugins=5; engineering-skills=42; hiker-skills=9; desktop=1.3.0; desktop-skills=5; total-skills=56 -->
...
| 智能软件工程平台 | 5.13.0 | 5 个插件、42 个原子 Skill | 从零开发、架构反证、存量源码接管、B/S、C/S、超长会话、大型工程、长链路诊断、不可变审核、Git、质量和发布 | ChatGPT语义选择、规则守门、其余按需加载 |
...
这是当前主要的 ChatGPT/Codex 桌面端软件工程插件套件。它使用一个小型入口读取紧凑能力目录与有界项目证据，由 ChatGPT 理解真实意图并选择当前阶段能力，再由确定性守门器校验数量、阶段、架构证据、权限和源码身份。轻量路由不占额度，每阶段最多加载两个活跃原子 Skill；第三个及之后进入待执行队列，不预读完整 Skill 正文，也不静默丢失后续能力。
...
- **模型主导语义选择**：ChatGPT 区分当前要求、否定项、历史错误、示例和未来计划，并基于紧凑中文目录选择最多两个原子 Skill；脚本不再通过关键词决定 B/S、C/S、阶段或具体 Skill。
- **确定性规则只做守门**：守门器核验候选存在性、数量、阶段、Manifest/`.ai` 架构证据、权限和源码身份；冲突时返回诊断并要求模型重选，不会擅自换成另一个 Skill。
- **误触发安全退化**：没有模型提案时返回 `PROPOSAL_REQUIRED` 和空加载列表，即使请求包含 Android、Worktree、merge 或桌面等词，也不会自动加载错误能力。
...
- **当前阶段与未来队列分离**：模型明确当前唯一主动作，把后续审核、测试、合并和发布放入有界待执行队列，阶段变化后重新语义选择。
...
- **两阶段中文透明回执**：软件工程会话第一条助手输出先显示“智能工程轻量路由”，模型语义选择、守门校验和 Skill 完整读取后再显示本阶段实际加载的中文插件和中文 Skill；相同阶段不重复，阶段切换或上下文恢复时重显。

### 自动应用规则

只有“智能工程轻量路由”参与全局自动选择。进入后由 ChatGPT 根据用户语义和项目证据选择候选，确定性脚本只做守门；路由本身不计入额度，并最多加载两个活跃原子 Skill。其他插件和 Skill 不会因为已安装就自动进入上下文；后续阶段能力只进入有界待执行队列。
...
- [智能软件工程平台 5.13 验证报告](skill-groups/ai-software-engineering-platform-enterprise/VALIDATION_REPORT_CN.md)
