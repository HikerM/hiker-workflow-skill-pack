...
Hiker Engineering Capability System（Hiker 工程能力系统）运行在 ChatGPT Desktop / Codex 提供的 Agent Runtime 之上；它不是独立 Runtime，也不接入第二个模型。当前工程套件包含 5 个插件、42 个 Skill，发布源码对应 477 项源码测试。它是普通软件工程任务唯一允许自动进入的能力包。
