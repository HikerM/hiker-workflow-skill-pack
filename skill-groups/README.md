...
| 第二组 | `ai-software-engineering-platform-enterprise/` | 企业软件工程平台，5 个插件、17 个 Skill |
| 第三组 | `desktop-app-reconstruction-zh/` | 桌面软件等价重建，1 个大型 Skill |

不要将第二组的 17 个 Skill 平铺复制到第一组目录。第二组依赖插件清单、共享脚本、Hook 和 Marketplace 元数据，应保持完整插件包结构。第三组也应作为单一 Skill 整体安装，不能把其 `references/`、`scripts/` 或 `assets/` 拆散。
