...
- 42个Skill前置描述从约13.3KB压缩到10KB以内；桌面稳定性门禁新增全局模板、单描述、描述总量和Skill正文总量预算。
- 路由准入指纹增加目标修订、当前动作、仓库身份、HEAD、脏状态、Manifest与套件版本；可信状态命中后复用已加载Skill和回执。
- 新增五插件完整版本检查与精确同版本Skill定位，禁止旧核心从最新其他插件目录拼接混合版本。
- 项目路由记录保存套件版本和指纹；旧任务版本漂移时仅允许Checkpoint、上下文恢复与接管，普通开发被阻断。
- 个人安装器默认仅保留一个活动缓存版本，旧版进入可恢复备份，并明确旧任务必须停止写入后由新任务接管。
- 热上下文默认降到4000字符，活动摘要8000字符；近期/里程碑Checkpoint和关闭任务热索引同步收紧。
- 长会话增加75%软阈值，软阈值只推荐自然边界Checkpoint，硬阈值才强制轮换，避免过早进入治理阶段。
- 测试运行器支持按插件分片并生成源码指纹证据；发布收尾仍执行五插件全量测试和一致性审核。
