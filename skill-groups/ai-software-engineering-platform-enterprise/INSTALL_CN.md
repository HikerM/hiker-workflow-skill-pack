...
桌面端已经创建的任务可能保留创建时注入的指令和 Skill 正文，不能把“缓存文件已替换”冒充“旧任务已经热更新”。个人安装器默认只保留一个活动缓存版本，上一版本进入可恢复备份；安装完成后：

1. 正在写代码的旧任务先保存 Checkpoint 并停止写入；
2. 新任务核对五个插件的完整版本和套件指纹；
3. 用「上下文恢复」读取当前目标、Git、Task 与 Checkpoint 后接管；
4. 项目记录的套件指纹不一致时，普通开发路由会被阻断，直到完成恢复迁移；
5. 若新任务仍指向旧缓存，关闭并重启桌面端，再创建新任务复验。

这保证不会混合执行新旧规则，但不承诺旧任务能够原地替换上下文。
...
这里的“显示”是任务对话中的文本回执，不是桌面插件列表新增一个实时监控徽章。插件列表显示安装/启用状态，应用回执显示本次任务实际采用的插件和Skill。复杂任务若发生范围膨胀、结论失效、实现路径分叉或部署漂移，会另行显示去重的中文工程健康告警；它不改变插件列表，也不会在状态未变化时重复。
