...
当前套件包含 5 个插件、33 个原子能力。只有用户可见的“智能工程轻量路由”可隐式触发，其余32个能力手动可见并按需懒加载；内部英文ID只用于Codex兼容和文件定位，不出现在默认应用回执。0→1项目先建立稳定需求ID、冲突/未知账本和技术决策Checkpoint；已有部分源码的项目先建立证据化能力基线并做需求差异对账；大型工程继续由七类职责契约贯穿需求到发布。
...
- 浏览器端项目：智能工程核心 + 浏览器端界面与前端工程 + 工作区与多会话协作 + 质量、风险与发布
