...
def display(name:str)->str:return str(load(ROOT/"plugins"/name/".codex-plugin"/"plugin.json").get("interface",{}).get("displayName") or "未命名工程插件")
def main()->int:
    ...
    atomic(market,merged);print(json.dumps({"ok":True,"repository":str(repo),"plugins":[display(name) for name in PLUGIN_NAMES],"marketplace":str(market),"backup":str(backup) if backup.exists() else None},ensure_ascii=False,indent=2));return 0
