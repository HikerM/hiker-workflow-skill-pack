...
只把本文件作为候选元数据读取；不要预读任何原子 `SKILL.md`。根据用户当前动作、否定语义、未来阶段和项目证据，由 ChatGPT 选择当前阶段最多两个候选。脚本只校验候选，不负责理解用户意图。
...
1. 先确定当前动作，不把“完成后审核、测试、推送”当作当前阶段。
2. 明确分离正向要求、否定项、示例、历史错误和未来计划；出现技术名不等于要采用该技术。
3. 项目已有工程证据时以证据为准；证据不足写 `unknown`，不猜最新版或架构。
4. 当前阶段最多两个候选；未来能力进入最多八项待执行队列，阶段变化后重新选择。
5. 选择后提交守门器校验；被拒绝时根据诊断重新选择，禁止让脚本改选其他 Skill。
