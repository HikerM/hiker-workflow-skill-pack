...
description: AI软件工程三十二个原子Skill的唯一轻量自动入口。收到从0开始、接管现有项目、B/S、C/S、Unity、测试、审核、Git、多会话或发布请求时，先用确定性脚本识别项目模式、架构和阶段，只读取最多两个直接相关的原子Skill，并展示应用回执；不得扫描或预加载完整Skill目录。
---
