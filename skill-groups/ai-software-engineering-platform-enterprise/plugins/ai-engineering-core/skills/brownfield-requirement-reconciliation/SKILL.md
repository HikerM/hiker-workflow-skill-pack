...
description: 已有部分工程源码、半成品、遗留系统或二次开发项目需要融合新增自定义需求时，基于真实代码、配置、接口、数据库和测试证据建立存量能力基线，将新需求分类为新增、修改、替换或移除，生成影响矩阵、冲突、未知项和迁移Checkpoint。不得重新套脚手架、擅自替换技术栈或把源码现状冒充业务需求。
---
...
5. 先解决阻塞冲突，再进入任务拆解。涉及架构、公共API、数据库兼容、安全边界或不可逆迁移时建立人工 Checkpoint；普通实现细节继续执行。
6. 聊天只读取 `REQUIREMENT_DELTA.md`；完整基线、哈希、需求历史和对账结果留在 `.ai/requirements/`。
...
- 每个非新增需求指向有效 `CAP-*`；
- 新需求具备验收条件和影响范围；
- 架构、API、数据、安全与迁移风险已进入Checkpoint或任务计划；
- 未用重写工程掩盖存量兼容问题。
