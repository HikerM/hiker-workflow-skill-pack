...
description: 从空目录或零代码开始开发自定义B/S、C/S或混合软件时，建立可增量合并的需求账本、冲突与未知项、验收条件、架构候选和自动决策检查点；根据真实约束选择技术及版本，不套固定脚手架，不设置人工审批暂停点，也不在关键决策记录前开始大规模编码。
---
...
- 技术候选比较与一个自动决策 Checkpoint；记录候选、证据、风险、回退和选择后自动锁定架构与精确版本并继续，不弹出审批。
...
5. 对冲突和未知项分级。会改变平台、架构、部署、数据、安全边界或核心技术栈的选择必须自动生成决策 Checkpoint；该记录不是审批请求，写入后继续处理。只有缺少决策必需的外部事实或权限时才请求用户补充信息。
...
8. 依据已知约束自动选择推荐方向，查对应官方文档核验受支持版本，将候选、选择理由、证据、风险、回退和精确版本写入决策记录；随后自动初始化脚手架并转入标准任务生命周期。用户后续可调整方向，但 Checkpoint 只能是非阻塞记录。
9. 聊天只加载 `REQUIREMENTS.md` 活动切片和当前决策，不加载整个 revision history。

## 权限

- 允许写需求、架构候选、项目状态和自动决策 Checkpoint。
- 关键选择尚未形成可追踪决策记录前只允许做小型验证性 POC；记录完成后自动继续。不可逆迁移、生产写入、部署或发布仍遵守各自权限门禁。
