...
6. 根据约束生成最多三个候选，比较兼容性、交付风险、长期维护、生态、性能、部署成本和团队接手成本。不得按流行度直接决定。
7. 技术方向确认后查对应官方文档核验受支持版本，将证据和精确版本写入决策记录；随后再初始化脚手架并转入标准任务生命周期。
8. 聊天只加载 `REQUIREMENTS.md` 活动切片和当前决策，不加载整个 revision history。
