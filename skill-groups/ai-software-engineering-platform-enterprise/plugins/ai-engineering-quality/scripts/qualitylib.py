...
from source_surface import TraversalLimitReached,bounded_process_run,iter_git_nul_records,is_reserved_source_path

def now()->str:return datetime.now(timezone.utc).isoformat(timespec="seconds")
def run(root:Path,*args:str,check:bool=True)->subprocess.CompletedProcess[str]:return bounded_process_run(list(args),root,check=check)
...
def worktree_fingerprint(root:Path)->str:
    ...
    try:
        for row in iter_git_nul_records(root,["status","--porcelain=v1","-z","--untracked-files=all","--",".",":(exclude).ai/**"]):
            ...
            p=root/path
            if is_reserved_source_path(root,p):continue
