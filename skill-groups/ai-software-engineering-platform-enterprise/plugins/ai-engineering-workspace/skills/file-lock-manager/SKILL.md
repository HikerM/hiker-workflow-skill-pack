...
```bash
python <plugin-root>/scripts/file_lock.py --root . acquire --task-id KG-001 --agent-role "WRITE" --owner writer-a --paths Assets/Main.unity Assets/Main.unity.meta
...
只允许 Gate Applicability 已确认存在有界写活动、且处于允许写入状态边界的任务持锁；只读 ASSURE 不持写锁。锁冲突时禁止写入；不得删除他人锁、伪造任务 ID 或让暂停任务继续写。异常接管必须由 CONTROL 核对 checkpoint、任务状态和持有者后处理。旧版 Developer/Master 标签只用于兼容映射，不是 Agent 创建理由。
