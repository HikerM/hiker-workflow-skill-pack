...
description: 为多Worktree写任务提供Git公共目录级文件锁，保护Unity Scene、Prefab、ProjectSettings、meta及NodeTS核心Service、数据库迁移、API Contract。用于并行开发前的锁定、冲突检查、心跳和释放。
---

# 文件锁管理

锁存储在 Git common dir，所有 Worktree 共享。Unity 资产与 `.meta` 视为同一资源；`ProjectSettings`、数据库迁移和 API Contract 采用全局互斥。NodeTS 核心 `*Service.ts` 至少采用文件级互斥。
