...
description: 为跨模块、跨仓库、长时间、多轮修复、真实外部执行、部署回滚或反复被用户纠正的复杂工程任务建立有界收敛控制。用于压制范围膨胀、同一职责新旧实现并存、合同测试冒充用户效果、旧结论在需求或证据变化后继续沿用、连续失败仍叠加补丁、生产版本与Git主线漂移，并在重要状态变化时向用户输出去重的中文工程健康告警。
---
...
连续两个治理周期没有业务价值增量时，停止继续增加投影、摘要、交叉摘要或全量矩阵。第三个治理周期必须先复用未失效证据、缩小到首个真实阻断，或记录明确的继续理由；不得反复声称“最后一道门”，新发现改变路径时必须更新完整剩余门禁。
