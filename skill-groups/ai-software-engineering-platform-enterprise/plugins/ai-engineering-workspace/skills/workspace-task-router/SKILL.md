...
- 纯后端请求也必须识别真实语言、框架、运行时、包管理器和版本；只读取命中的一个后端参考：Node.js/TypeScript 见 [Node.js与TypeScript后端](references/backend-node-typescript.md)，.NET 见 [.NET后端](references/backend-dotnet.md)，JVM 见 [JVM后端](references/backend-jvm.md)，Python 见 [Python后端](references/backend-python.md)，Go/Rust/PHP/Ruby 等见 [其他后端](references/backend-other.md)。
...
- 后端实现先执行变更契约和公共接口影响检查；Service、数据库迁移、API/事件契约、鉴权和队列消费者不得由多个写通道并行修改。
