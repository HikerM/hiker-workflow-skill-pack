...
- 软件工程开发、设计、测试、Git、多会话或发布任务只允许自动进入第二组“智能工程轻量路由”。第一组工作流技能与第三组桌面应用复刻技能只有用户明确点名或手动选择时才启用；不得自动触发。一次只选一个主路由，原子能力由主路由按需读取。
...
- 首次实质动作前显示“插件应用回执”：插件中文名称、实际加载的 Skill 中文名称、原因、项目/仓库、模式；插件名和 Skill 名不得显示英文内部名称。结束时只汇总实际应用项。
