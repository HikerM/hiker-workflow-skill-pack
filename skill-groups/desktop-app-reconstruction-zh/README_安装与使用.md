...
2. 若界面提供“从电脑导入／上传 Skill”，选择整个 `desktop-app-reconstruction-zh-v1.1.0.zip`，不要只上传 `SKILL.md`；
...
Skill 内部名称仍为 `desktop-app-reconstruction-zh`，因此 v1.1.0 可以覆盖安装 v1.0.0。安装脚本默认备份旧 Skill。

旧的重建项目不会被自动改写。处理旧项目时：

1. 先备份旧项目；
2. 使用 v1.1.0 初始化一个新项目骨架；
3. 迁移原证据、规格、源码和测试；
4. 补齐新库存、精确版本锁、覆盖矩阵、追踪矩阵和交付物清单；
5. 运行 v1.1.0 门禁，未通过前不要沿用旧版“完成”结论。
...
```bash
python scripts/validate_skill_package.py desktop-app-reconstruction-zh-v1.1.0.zip --self-test
```
